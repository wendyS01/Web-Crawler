...
final class ProfilerImpl implements Profiler {
  ...
  public <T> T wrap(Class<T> klass, T delegate) throws IllegalArgumentException{
    ...
    Stream<Method> methods = Arrays.stream(klass.getMethods());
    if(methods.noneMatch(x -> x.isAnnotationPresent(Profiled.class))) {
      throw new IllegalArgumentException("Class does not contain profiled method");
    }
    T proxy = (T) Proxy.newProxyInstance(delegate.getClass().getClassLoader()
